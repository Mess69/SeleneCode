...
pub(crate) async fn run_router(
	...
	// SELENE PATCH (the crate's one divergence from crates.io 3.2.1): seed the
	// datastore ConfigMap from `SURREAL_*` env vars, exactly as the server
	// binary does. Without this the embedded SDK runs `ConfigMap::empty()` and
	// every `SURREAL_ROCKSDB_*`/`SURREAL_DATASTORE_*` variable is silently
	// ignored — on a 32 GiB machine that means a 15 GiB block-cache allowance
	// and 128 MiB x 8 write buffers for a laptop indexer. Endpoint query params
	// still win over env (ConfigMap::join prefers the joined-in values).
	let builder = Datastore::builder()
		.with_config(surrealdb_core::cnf::ConfigMap::from_env())
