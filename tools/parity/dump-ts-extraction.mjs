...
/**
 * The codegraph SHA this baseline was generated from — the baseline's
 * provenance, and the one thing that makes a stale baseline detectable.
 *
 * Read it from git rather than trusting the operator to export
 * `CODEGRAPH_COMMIT` (the env var still wins, for CI or a detached checkout).
 * Forgetting the export silently stamped `"unknown"`, which `baseline_is_not_vacuous`
 * rejects — a real trap that cost a regeneration cycle. Derive it, don't ask for it.
 */
function codegraphCommit() {
  if (process.env.CODEGRAPH_COMMIT) return process.env.CODEGRAPH_COMMIT;
  try {
    return execFileSync('git', ['-C', CG, 'rev-parse', 'HEAD'], {
      encoding: 'utf8',
    }).trim();
  } catch {
    return 'unknown'; // not a git checkout — the gate will reject this
  }
}
...
const payload = {
  ...
  codegraphCommit: codegraphCommit(),
